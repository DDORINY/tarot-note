...
function dbReadingToDetail(reading: DbReading): Extract<DetailState, { status: "ready" }> {
  ...
  const diary = reading.diary_entries?.[0];
  ...
  return {
    ...
    result: {
      ...
      keyMessage: "저장된 카드들의 포지션과 방향을 기준으로 핵심 흐름을 다시 살펴보세요.",
      connection: "상세 기록에서는 각 카드가 맡은 위치와 해석을 이어 읽으며 당시의 고민 흐름을 복기할 수 있습니다.",
      current: "현재 상황은 카드별 해석과 포지션 의미를 함께 볼 때 더 선명해집니다.",
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm text-gold">
            {detail.category} · {detail.spreadName}
          </p>
          <h1 className="mt-2 text-3xl font-semibold text-softGold">{detail.question}</h1>
          <p className="mt-2 text-sm text-mist">{new Date(detail.createdAt).toLocaleString("ko-KR")}</p>
        </div>
      ...
      {(detail.diary?.emotion || detail.diary?.note) && (
        <Card>
          <h2 className="text-lg font-semibold text-softGold">감정 메모</h2>
