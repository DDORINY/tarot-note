        ...
        <Card key={spread.id} className="flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-mist">{spread.description}</p>
            </div>
            {spread.isPremium && <Lock className="shrink-0 text-gold" size={18} />}
          </div>
          <Button variant={spread.isPremium ? "secondary" : "primary"} onClick={() => selectSpread(spread)}>
            {spread.isPremium && !ENABLE_PREMIUM_BYPASS ? "프리미엄 보기" : "선택하기"}
          </Button>
        </Card>
      ))}
    </div>
